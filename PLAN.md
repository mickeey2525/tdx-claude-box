...
- Claude Code(native 2.1.201)のブラウザ起動挙動(実機確認):
  ...
  - 3118 はホスト上で全 box の取り合いになるため共有ポートとして振り分ける:
    コールバックサーバーは認証プロンプト表示中しか listen しない性質を使い、
    接続ごとに自 box → 他の実行中 box の順に probe して届け先を決める。
    ポートを取れなかったセッションは 15 秒毎に再試行して引き継ぐ。
    OAuth の state/PKCE により誤配送しても資格情報は漏れない(拒否されるだけ)
